...
CameraXやMediaPipe SDK objectをFSMへ持ち込まず、fakeable adapter input、`LowerBodyPose`、`PoseFeatureSample`を段階別にpure Kotlinで検証する。
...
Phase 9と最終Camera bugfixはpure KotlinでMediaPipe adapter boundary、One-Euro Filter、angle / quality / side stickiness、320×240優先のanalysis resolution policy、Bitmap変換前のanalysis throttle、RGBA row padding、Emulator判定、GPU no-callback watchdogとFSMを検証する。Camera matrixにはinvalid timestamp / busy / throttle時にconversion callbackを呼ばずframeをcloseすること、callbackまでsingle in-flight、4 / 8 / 10 FPS設定、Bitmap / rotation allocation countを含める。FSM matrixにはBOTTOMのknee-only / knee+hip / hip+reversal経路、膝またはhip signalの一時欠損、不規則2/4/5/8 FPS、700ms / Emulator 2,500ms gap、Emulator 3,900/4,000ms超とphysical 2,000ms超のpose loss、2 sample / provisional / auto-descent / soft-timeout Calibration、confidence / size fallback、500〜1,300ms gap、実Camera角度系列、1/10 reps、浅い屈伸、前屈、深い動作、bottom bounce、threshold jitter、順序不正、重複・逆順frame、minimum/maximum durationを含める。Flutterではversion mismatch、malformed / private payload、permission、model loading、`awaitingResult` / `noPose`分離、session固定、native rep重複、Calibration診断、Outbox、terminal Debt停止、start中route離脱を検証する。Android instrumentationではCAMERA宣言、同梱Lite assetからのMediaPipe CPU detector初期化、debug生成fixtureのcallback / pose / hip-knee-ankle、Native Preview/guideのbounds一致、attach/detach、debug synthetic sequenceを検証する。EmulatorはCPU固定とし、GPU delegateと実Camera latencyは物理端末で別途確認する。
