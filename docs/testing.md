...
Phase 9と最終Camera bugfixはpure KotlinでMediaPipe adapter boundary、One-Euro Filter、angle / quality / side stickiness、analysis throttleとFSMを検証する。FSM matrixには1/2/10/100 reps、浅い屈伸、bottom bounce、threshold jitter、順序不正、100ms/300ms tracking loss、frame gap、重複・逆順frame、500ms/7秒動作を含める。Flutterではversion mismatch、malformed / private payload、permission、model loading、session固定、native rep重複、診断state分離、Outbox、terminal Debt停止、start中route離脱を検証する。Android instrumentationではCAMERA宣言、同梱Lite assetからのMediaPipe CPU detector初期化、PlatformView相当のattach/detach、debug synthetic sequenceを検証する。GPU delegateと実Camera latencyはEmulator testの成功だけで物理端末達成扱いにしない。
