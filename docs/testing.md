...
Phase 9と最終Camera bugfixはpure KotlinでMediaPipe adapter boundary、One-Euro Filter、angle / quality / side stickiness、analysis throttleとFSMを検証する。FSM matrixには1/10/100 reps、浅い屈伸、過深動作、bottom bounce、threshold jitter、順序不正、短期/長期tracking loss、frame gap、重複・逆順frame、minimum/maximum durationを含める。Flutterではversion mismatch、malformed / private payload、permission、model loading、session固定、native rep重複、診断state分離、Outbox、terminal Debt停止、start中route離脱を検証する。Android instrumentationではCAMERA宣言、同梱Lite assetからのMediaPipe CPU detector初期化、Native Preview/guideのbounds一致、attach/detach、debug synthetic sequenceを検証する。GPU delegateと実Camera latencyはEmulator testの成功だけで物理端末達成扱いにしない。
